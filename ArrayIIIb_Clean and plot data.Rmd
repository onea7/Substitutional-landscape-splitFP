...
n_rm = 0
for (col in sec_col) {
    # mask = apply(d[,c('mean','sd',col)], MARGIN=1, function(a) { (a[3]-a[1])/a[2] > 3})     # 3 std. dev
    mask = apply(d[,c('median','uncert',col)], MARGIN=1, function(a) { (a[3]-a[1])/a[2] > 6})    # 3*2 MAD, the factor of 2 is the conservative choise
...
print(sprintf("Total %d outliers removed",n_rm))
```
...
color = 1
#plot(0,0,col="white",xlim=c(min(breaks),max(breaks)), ylim=c(0,.005))
...
p <- ggplot(d_clean, aes(x=d_clean$w, y=d_clean$mean))
...
p +  geom_jitter(position=position_jitter(0.3)) +
     ...
     # geom_hline(yintercept=201, linetype="dashed", color = "grey", size=0.5) + # mean s10 short
     xlab("\nPeptide format") + 
     ...
     stat_summary(fun.y=median, geom="point", shape=20, size=2, color ="red") +
     theme_classic() +
...
```{r}
  ...
  mtext(text = "Mean fluorescence
      short format, all substitution variants",
        ...
        cex=1.8)
  abline(0,1)
  coll=c("darkorange1",3)
  points(dd$F1, dd$F2, pch=20, cex=.6, col=coll[dd$li1])
  ddi = which(substr(dd$seq1,10,10)=="R")
  points(dd[ddi,'F1'], dd[ddi,'F2'], pch=20, cex=.3, col=4)
  legend("bottomright",c("Neutral linker","No linker","L207R both linkers","slope = 1"), 
         pch=c(20,20,20,NA), lty=c(NA,NA,NA,1), col=c("darkorange1",3,4,1), cex=1.6)
...
i1 = di[i[,1]]
i2 = di[i[,2]]
dd1 = data.frame(seq1=d[i1,'seq'], seq2=d[i2,'seq'], w=droplevels(d[i1,'w']), F1=d[i1,'mean'], F2=d[i2,'mean'], li1=droplevels(d[i1,'link']), li2=droplevels(d[i2,'link']), d1=d[i1,'d'], d2=d[i2,'d'])
# Data set 2
...
i1 = di[i[,1]]
i2 = di[i[,2]]
dd2 = data.frame(seq1=d[i1,'seq'], seq2=d[i2,'seq'], w=droplevels(d[i1,'w']), F1=d[i1,'mean'], F2=d[i2,'mean'], li1=droplevels(d[i1,'link']), li2=droplevels(d[i2,'link']), d1=d[i1,'d'], d2=d[i2,'d'])
# Data set 3
...
i1 = di[i[,1]]
i2 = di[i[,2]]
dd3 = data.frame(seq1=d[i1,'seq'], seq2=d[i2,'seq'], w=droplevels(d[i1,'w']), F1=d[i1,'mean'], F2=d[i2,'mean'], li1=droplevels(d[i1,'link']), li2=droplevels(d[i2,'link']), d1=d[i1,'d'], d2=d[i2,'d'])
...
context.heatmap <- ggplot(data = st_gathered_ALL, mapping = aes(x = factor(Context),
                                                         y = st_gathered_ALL$Substitution,
                                                         fill = log2(Fluorescence))) +
  geom_tile(color = "white") +
  #geom_rect() +
  xlab(label = "Sequence") +
...
i1 = di[i[,1]]
i2 = di[i[,2]]
dd1 = data.frame(seq1=d[i1,'seq'], seq2=d[i2,'seq'], F1=d[i1,'mean'], F2=d[i2,'mean'], li1=droplevels(d[i1,'link']), li2=droplevels(d[i2,'link']), d1=d[i1,'d'], d2=d[i2,'d'])
...
i1 = di[i[,1]]
i2 = di[i[,2]]
dd2 = data.frame(seq1=d[i1,'seq'], seq2=d[i2,'seq'], F1=d[i1,'mean'], F2=d[i2,'mean'], li1=droplevels(d[i1,'link']), li2=droplevels(d[i2,'link']), d1=d[i1,'d'], d2=d[i2,'d'])
...
i1 = di[i[,1]]
i2 = di[i[,2]]
dd3 = data.frame(seq1=d[i1,'seq'], seq2=d[i2,'seq'], F1=d[i1,'mean'], F2=d[i2,'mean'], li1=droplevels(d[i1,'link']), li2=droplevels(d[i2,'link']), d1=d[i1,'d'], d2=d[i2,'d'])
...
i1 = di[i[,1]]
i2 = di[i[,2]]
dd4 = data.frame(seq1=d[i1,'seq'], seq2=d[i2,'seq'], F1=d[i1,'mean'], F2=d[i2,'mean'], li1=droplevels(d[i1,'link']), li2=droplevels(d[i2,'link']), d1=d[i1,'d'], d2=d[i2,'d'])
```
...
dexp <- d[d$seq %in% c("LPDNHYLSTQTVRSKDPN","LPDNHYLSTQTVLSKDPN","LPDNYYLSTQTVLSKDPN","LPDNHYLSYQTVLSKDPN","LPDNYYLSYQTVLSKDPN"),]
  ...
  #tiff("exp-array.tiff", units="in", width=12, height=15, res=500) 
    library(ggplot2)
  library(ggbeeswarm)
  ggplot(data=dplot_m, aes(x=variant,y=dplot_m$value, colour=variant)) +
    geom_boxplot(lwd=2) +
    geom_jitter() +
